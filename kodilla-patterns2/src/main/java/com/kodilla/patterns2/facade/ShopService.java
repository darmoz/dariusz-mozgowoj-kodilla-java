...
public class ShopService {
    ...
    public void addItem(Long orderId, Long productId, double qty) {
        orders.stream()
                ...
                .forEach(o -> o.getItemList().add(new Item(productId, qty)));
    }
    ...
    public boolean removeItem(Long orderId, Long productId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            int orderSize = theOrder.getItemList().size();
            for (int n = 0; n < orderSize; n++) {
                if (theOrder.getItemList().get(n).getProductId().equals(productId)) {
                    theOrder.getItemList().remove(n);
                    return true;
                }
            }
        }
